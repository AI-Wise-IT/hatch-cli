...
- [x] 6.4 Nothing is written back into `intake/`. The `intake/backlog-*` records are retired in favour of unfinished OpenSpec proposals, so there is no record to mark picked up; and `intake/mvp-scope.md` is retired too, its standing "No" decisions now read from the PRD's own No list (`intake/product-requirements.md`), which this change's citations point at. This change is itself the record that the deferred list/browse item was taken up.
