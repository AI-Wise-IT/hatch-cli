...
That model is right after launch. The audit changes only the transition: before launch, real names may still be corrected once, but only through an explicit cleanup review and immediate hardening follow-up.
...
- Make the final audit a deliberate release-readiness activity for every current skill/group.
...
- Applying real-content permanence rules to `_`-prefixed fixture cleanup.
...
Those warnings are not failures during the audit, but they are not ignored either. A cleanup PR that triggers them must review each warning as an intentional launch cleanup action. This keeps the exception reviewable without pretending the long-term rule has disappeared.
...
- **Warnings can be normalized** -> Require every real-content warning to be reviewed as an intentional launch cleanup action; unexplained warnings mean the cleanup PR is not ready.
...
1. Audit every current top-level folder in `hatch-skills` and classify the launch disposition of each registry entry.
2. Apply cleanup PRs while `NAME_PERMANENCE_ENFORCEMENT` remains `warn`.
3. For each cleanup PR, verify all blocking checks pass and every name-permanence warning is intentional.
