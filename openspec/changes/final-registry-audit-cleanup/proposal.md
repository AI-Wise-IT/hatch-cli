...
- **Define a final registry audit before launch.** The audit enumerates every current top-level registry entry, classifies it as keep, rename, remove, fixture-only, or defer, and records the reason for every rename/removal decision.
...
- **Improve `prd-elicitation` as content, not as spec.** Feedback on its conversational flow is handled in the skill file and audit ledger. The specs capture the convention a skill must satisfy, not the PRD skill's exact interviewing process.
- **Allow real-content renames and deletions only during that audit window.** The name-permanence CI check remains capable of warning without blocking while the final cleanup PRs land. Warnings are expected during those PRs, but they must be matched by an audit ledger entry.
...
- Add an audit ledger, likely under `docs/launch-registry-audit.md` or a similarly durable repo-local document, that records each keep/rename/remove/defer decision and any required follow-up.
