...
**Cross-repository context does not deliver record *text* to `hatch-skills`.** → **Settled: it does not.** `context.repos` names `AI-Wise-IT/hatch-cli` and the setting is accepted, but asked directly whether it could read a named record file, the reviewer answered: "I cannot reach docs/architecture/decisions/0013-registry-group-structure-and-permanence.md in the AI-Wise-IT/hatch-cli repository." An earlier review reported the same thing unprompted. The fallback is therefore in force: that repository's rules cite records by permanent URL. The no-copy requirement holds either way; what degrades is how much the reviewer can read, not where the source of truth lives. `rules.md` there instructs the reviewer to report an unreachable record rather than infer its content from the rule text, which it has been observed doing.
