...
- **Pre-launch hardening is always the last batch, once any ADR carries an Invariants entry.** Symmetric to infrastructure-first, but conditional rather than universal: not every project has a rule that becomes permanent once a real dependent exists, but the moment one does, ending the plan with a hardening batch stops being a judgment call — it's the only place in the sequence where "flip enforcement to blocking, resolve what's still safe to clean up" can run against a build that's otherwise finished, the same way Batch 1 can only run against a build that hasn't started yet. Don't schedule it earlier speculatively (there's nothing to harden until the rest of the build exists) and don't leave it off the plan by default (an invariant with no hardening batch scheduled is exactly the drift risk `pre-launch-audit`'s repeated-run design exists to catch — better to schedule it and let the developer defer it explicitly than to never name it). If the project has adopted `pre-launch-audit`/`pre-launch-harden`, this batch is literally those two skills run in sequence; if it hasn't, state the same requirement in the batch's own terms so it isn't silently skipped.
