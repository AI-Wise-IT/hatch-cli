...
- That credential is the `HATCH_SKILLS_READ_TOKEN` repo secret on hatch-cli: a fine-grained personal access token scoped to `AI-Wise-IT/hatch-skills` only, with Contents: Read-only permission and no other scope granted. Any replacement or rotation must keep that scope — a broader token would give this repo's CI write reach into the registry it is only meant to read.
