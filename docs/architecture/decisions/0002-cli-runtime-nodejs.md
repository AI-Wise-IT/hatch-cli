...
`hatch new` and `hatch import` (UC-1, UC-3, UC-4) perform real git plumbing against arbitrary target projects: auto-init if missing, exactly one commit per operation, and rollback-to-last-commit on partial failure. Every one of today's dev sandboxes (local and cloud-agent) already has a system `git` binary present, making shell-out the lowest-risk path.
...
- Installation documentation must state a system `git` installation as a prerequisite.
