...
- **Failure surface:** a group whose *member* depends on something removed is not caught until the registry-side mechanism exists — an accepted, explicitly-scoped gap, not silently dropped (see Context, and the unfinished proposal named in Consequences).
...
- Two related, still-unbuilt capabilities are recorded rather than designed here: a registry-side CI check that a group's *current* version doesn't depend on a removed skill (`openspec/changes/add-removed-dependency-check/`), and a project-level sync-everything command (`openspec/changes/add-project-sync-command/`). Both are unfinished proposals — a rationale and open questions, nothing scoped.
...
- MUST NOT apply this block merely because a *member* of an otherwise-fine group is removed — that remains warn-only pending the registry-side dependency-detection mechanism recorded in Consequences.
