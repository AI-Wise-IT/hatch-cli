...
UC-4 ([`docs/use-cases/remove-content.md`](../../use-cases/remove-content.md)) describes AF-2 and AF-3 in prompt-shaped terms ("if run interactively: system asks... if run unattended: system aborts by default"), and this work's own brief proposed reusing `hatch import`'s AF-6 `promptLine` confirm pattern for consistency. Two things made that reuse plan wrong once examined:
