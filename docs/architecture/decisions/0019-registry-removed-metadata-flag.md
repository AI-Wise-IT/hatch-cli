...
[0013-registry-group-structure-and-permanence](0013-registry-group-structure-and-permanence.md) settled the *semantics* of "removed" — a metadata flag on the still-existing folder, never a deletion, explicitly stating "the flagged item's status changes" — but its own Consequences section left the concrete field name/shape unfixed, the same kind of gap [0016-group-member-manifest-format](0016-group-member-manifest-format.md) resolved for a group's `members` list. The re-import work flagged this directly: real registry content is needed to test against, so the format had to be settled before a 'removed' fixture could be published to `hatch-skills`.
